 ...
 * parsers act by shifting input onto a parseTree stack until the Symbols
 ...
 * on top of the parseTree stack (stored as part of a Symbol object representing
 * a terminal or non terminal).  The parseTree action table is consulted
 ...
 * <p>
 * This class actually provides four LR parsers.  The methods parseTree() and
 ...
 * has worked), and the other is used to parseTree through buffered "parseTree ahead"
 ...
 * the actual parseTree tables, generated code also supplies methods which
 ...
 * <dd> Provides a reference to the parseTree action table.
 ...
 * the point of a reduce in the parseTree, so this code also allocates and
 ...
 * report_fatal_error("Couldn't repair and continue parseTree", null);
 * </dl>
 *
...
public abstract class lr_parser {
    ...
    /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/
    /* Global parseTree state shared by parseTree(), error recovery, and
     * debugging routines */
    /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/
     ...
     * @param stack   the parseTree stack of that object.
     * @param top     the index of the top element of the parseTree stack.
     */
    public abstract java_cup.runtime.Symbol do_action(
            int act_num,
            lr_parser parser,
            Stack<Symbol> stack,
            int top)
     ...
     * any parseTree actions. This is filled in by generated code to create
    ...
    public java_cup.runtime.Symbol scan() throws Exception {
        java_cup.runtime.Symbol sym = getScanner().next_token();
    ...
    public void report_error(String message, Object info) {
        ...
        if (info instanceof java_cup.runtime.Symbol)
            if (((java_cup.runtime.Symbol) info).left != -1)
                System.err.println(" at character " + ((java_cup.runtime.Symbol) info).left +
                        " of input");
    ...
    public void unrecovered_syntax_error(java_cup.runtime.Symbol cur_token)
            throws Exception {
        report_fatal_error("Couldn't repair and continue parseTree", cur_token);
    }
    ...
    public java_cup.runtime.Symbol parse() throws Exception {
        ...
        java_cup.runtime.Symbol lhs_sym = null;
        ...
        for (_done_parsing = false; !_done_parsing; ) {
            ...
            /* finally if the entry is zero, we have an error */
                ...
                if (!error_recovery(false)) {
                    ...
                    /* just in case that wasn't fatal enough, end parseTree */
                    done_parsing();
                } else {
                    lhs_sym = stack.peek();
                }
    ...
    /**
     * Dump the parseTree stack for debugging purposes.
     */
    ...
    public void debug_stack() {
        ...
        for (int i = 0; i < stack.size(); i++) {
            java_cup.runtime.Symbol s = stack.elementAt(i);
    ...
    /**
     * Perform a parseTree with debugging output.  This does exactly the
     * same things as parseTree(), except that it calls debug_shift() and
     * debug_reduce() when shift and reduce moves are taken by the parser
     * and produces various other debugging messages.
     */
    public java_cup.runtime.Symbol debug_parse()
            throws Exception {
        /* the current action code */
        int act;
     ...
     * normal parser to continue with the overall parseTree.
     *
    ...
    protected boolean error_recovery(boolean debug)
	 ...
	 do that shift (if that fails, we fail) */
        ...
        /* read ahead to create lookahead we can parseTree multiple times */
        read_lookahead();

        /* repeatedly try to parseTree forward until we make it the required dist */
        for (; ; ) {
            /* try to parseTree forward, if it makes it, bail out of loop */
            if (debug) debug_message("# Trying to parseTree ahead");
        ...
        /* we have consumed to a point where we can parseTree forward */
        if (debug) debug_message("# Parse-ahead ok, going back to normal parseTree");

        /* do the real parseTree (including actions) across the lookahead */
     ...
     * state currently on the top of the (real) parseTree stack.
     */
    ...
    protected boolean find_recovery_config(boolean debug) {
        java_cup.runtime.Symbol error_token;
        int act;

        if (debug) debug_message("# Finding recovery state on stack");

        /* Remember the right-position of the top symbol on the stack */
        java_cup.runtime.Symbol right = stack.peek();// TUM 20060327 removed .right
        java_cup.runtime.Symbol left = right;// TUM 20060327 removed .left
    ...
    /**
     * Lookahead Symbols used for attempting error recovery "parseTree aheads".
     */
    protected java_cup.runtime.Symbol lookahead[];

    /**
     * Position in lookahead input buffer used for "parseTree ahead".
     */
    ...
    /**
     * Read from input to establish our buffer of "parseTree ahead" lookahead
     * Symbols.
     */
    protected void read_lookahead() throws Exception {
        /* create the lookahead array */
        lookahead = new java_cup.runtime.Symbol[error_sync_size()];
    ...
    /**
     * Return the current lookahead in our error "parseTree ahead" buffer.
     */
    ...
    /**
     * Advance to next "parseTree ahead" input Symbol. Return true if we have
    ...
    /**
     * Reset the parseTree ahead input to one Symbol past where we started error
    ...
    /**
     * Do a simulated parseTree forward (a "parseTree ahead") from the current
     * stack configuration using stored lookahead input and a virtual parseTree
     ...
     * parseTree() using only our saved "parseTree ahead" input, and not executing any
     * actions.
     *
     * @param debug should we produce debugging messages as we parseTree.
     */
    protected boolean try_parse_ahead(boolean debug)
        ...
        /* create a virtual stack from the real parseTree stack */
        virtual_parse_stack vstack = new virtual_parse_stack(stack);

        /* parseTree until we fail or get past the lookahead input */
        for (; ; ) {
     ...
     * parser performs all actions and modifies the real parseTree configuration.
        ...
        while (!_done_parsing) {
            ...
            if (act > 0) {
                ...
                if (!advance_lookahead()) {
                    ...
                    /* scan next Symbol so we can continue parseTree */
    ...
    /**
     * Utility function: unpacks parseTree tables from strings
     */
